    ...
    private void splitContainer1_Panel2_Paint(object sender, PaintEventArgs e)
        ...
        private void tvwCargas_AfterSelect(object sender, TreeViewEventArgs e)
        {
            try
            {
                ...
                if (Nodo.ImageKey == "nada")
                {
                    MessageBox.Show("se dejo de seleccionar selecciono");

                    for (int i = 0; i < ArrayDesde.Count - 1; i++)
                    {                
                        if (ArrayDesde[i].ToString() == Vble.desde.ToString())
                        {
                            ArrayDesde.Remove(i);
                            MessageBox.Show("se elimino la coincidencia");
                        }
        ...
        private DataTable CargarRegistrosSecuencia2()
        {
            ...
            try
            {
                txSQL = "select C.Secuencia, C.conexionID, L.CodigoInt, C.Ruta, C.usuarioID, P.Apellido, C.titularID, P.Apellido, C.propietarioID, P.apellido " +
                        ...
                        "INNER JOIN localidades L ON L.CodigoPostal = C.CodPostalSumin " +
                        //"INNER JOIN conceptosdatos D on C.conexionID = D.conexionID " +
                        "Where (C.Secuencia >= " + ArrayDesde[1].ToString() + " and C.Secuencia <= " + ArrayHasta[1].ToString() + " and C.Ruta = " + Ru.Text + " and L.CodigoInt = " + Loc.Text + " and C.ImpresionOBS = " + 0 +  "or C.Secuencia >= " +  ArrayDesde[2].ToString() + " and C.Secuencia <= " + ArrayHasta[2].ToString() + " and C.Ruta = " + 1169 + " and C.ImpresionOBS = " + 0 + ")";               
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("DESDE: " + ArrayDesde[i].ToString());
            }
        }
    }
