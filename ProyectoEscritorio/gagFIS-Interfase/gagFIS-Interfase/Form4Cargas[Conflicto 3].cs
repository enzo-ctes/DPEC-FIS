...
namespace gagFIS_Interfase
{
    ...
    private void splitContainer1_Panel2_Paint(object sender, PaintEventArgs e)
                    ...
                    if (Nodo.Level == 4 || Nodo.Level == 3)        
                     {
                        ...
                        {
                            case 1:
                                //MessageBox.Show(ArrayLocalidad[0].ToString());                            
                                dataGridView1.DataSource = CargarRegistrosSecuencia();
                                label3.Text = dataGridView1.RowCount.ToString();
                                break;
                            case 2:
                               // MessageBox.Show(ArrayLocalidad[0].ToString() + " y " + ArrayLocalidad[1].ToString());
       ...
       private SQLiteConnection con;
        ...
        /// <returns></returns>
        private DataTable VerRegistrosConexion(int codconex)
        {
            MySqlDataAdapter da;
            MySqlCommandBuilder comandoSQL;
            string txSQL;
        ...
        public void cargartablaconexiones()
        {
            ...
            try
            {
                //esta direccion es donde se encuentra la bd el cual va a ser cargados con los registros que se filtraron del Treview
                con = new SQLiteConnection("Data Source=F:/Google Drive/MACRO INTELL - Software/gagFIS-Interfase/gagFIS-Interfase/dbFIS-DPEC.db");
                con.Open();//abrimos la bd para realizar el INSERT
        ...
        /// <summary>
        /// devuelve los registros seleccionados del treeviews que SE CARGARON a la tabla conexiones
        /// </summary>
        /// <returns></returns>
        ...
        /// <param name="numeroconexion"></param>
        /// <returns></returns>
        private DataTable VerRegistrosMedidores(int codconex)
        {
            MySqlDataAdapter da;
            MySqlCommandBuilder comandoSQL;
            string txSQL;
        ...
        /// <summary>
        /// //Funcion que contiene la consulta INSERT de SQLite para cargar los registros seleccionados a la tabla MEDIDORES 
        /// </summary>
        public void cargartablaMedidores()
        {
            //declaracióno de variables
            ...
            //asignación a variables locales para manejar en el INSERT
            conexionid = Convert.ToInt32(this.dataGridView3.CurrentRow.Cells[0].Value);
            periodo = Convert.ToInt32(this.dataGridView3.CurrentRow.Cells[1].Value);
            Orden = Convert.ToInt32(this.dataGridView3.CurrentRow.Cells[2].Value);
            Modelo = this.dataGridView3.CurrentRow.Cells[3].Value.ToString();
            Numero = this.dataGridView3.CurrentRow.Cells[4].Value.ToString();
            Multiplicador = Convert.ToInt32(this.dataGridView3.CurrentRow.Cells[5].Value);
            Digitos = Convert.ToInt32(this.dataGridView3.CurrentRow.Cells[6].Value);
            AnteriorFecha = this.dataGridView3.CurrentRow.Cells[7].Value.ToString();
            AnteriorEstado = Convert.ToInt32(this.dataGridView3.CurrentRow.Cells[8].Value);
            ActualFecha = this.dataGridView3.CurrentRow.Cells[9].Value.ToString();
            ActualHora = this.dataGridView3.CurrentRow.Cells[10].Value.ToString();
            ActualEstado = Convert.ToInt32(this.dataGridView3.CurrentRow.Cells[11].Value);
            TipoLectura = Convert.ToInt32(this.dataGridView3.CurrentRow.Cells[12].Value);
            ...
            try
            {
                ...
                txSQL = "INSERT INTO medidores ([ConexionID], [Periodo], [Orden], [Modelo], [Numero], [Multiplicador], [Digitos]," +
                    " [AnteriorFecha], [AnteriorEstado], [ActualFecha], [ActualHora], [ActualEstado], [TipoLectura]) " +
                    "VALUES ('" + conexionid + "', '" + periodo + "', '" + Orden + "', '" + Modelo + "', '" + Numero + "', '" + Multiplicador + "', '" + Digitos +"', '" + AnteriorFecha + "', '" + AnteriorEstado + "', '" + ActualFecha + "', '" + ActualHora + "', '" + ActualEstado + "', '" + TipoLectura +"')";
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            try {                
                if (this.dataGridView1.RowCount > 0)//verifica que en el datagridview1 existan registros para recorrerlos y generar la carga de las tablas
                {
                    ...
                    foreach (DataGridViewRow Fila in dataGridView1.Rows)//recorre cada registro de los nodos seleccionados
                    {                                             
                        codconex = Convert.ToInt32(Fila.Cells["conexionID"].Value);//obtengo conexionID de cada registro para cargar a la tabla Conexiones
                        dataGridView2.DataSource = VerRegistrosConexion(codconex);                                              
                        cargartablaconexiones();   
                    }
                    dataGridConex.DataSource = VerConexionesCargadas();//muestra los registros seleccionados que se cargaron a la tabla conexiones (SQLite)
                    //foreach (DataGridViewRow Fila in dataGridView1.Rows)//recorre cada registro de los nodos seleccionados
                    //{
                    //    codconex = Convert.ToInt32(Fila.Cells["Nº_Conexion"].Value);//obtengo conexionID de cada registro para cargar a la tabla Conexiones
                    //    dataGridView3.DataSource = VerRegistrosMedidores(codconex);
                    //    cargartablaMedidores();

                    //}
                    //dataGridMed.DataSource = VerMedidoresCargados();
                }
                else
                {
                        ...
                        if (tNd2.ImageKey != "nada")
                        {
                            ...
                            foreach (TreeNode tNd3 in tNd2.Nodes)
                            {
                                if (tNd3.ImageKey != "nada")
                                {
                                    clave = tNd3.Tag.ToString();
                                    Vble.Ruta = dcNodos[clave].Ruta;
                                    //MessageBox.Show(Vble.Ruta.ToString(), "RUTA");
                                    //ArrayRuta.Add(Vble.Ruta.ToString());
                                    
                                }
                                foreach (TreeNode tNd4 in tNd3.Nodes)
                                {
                                    //MessageBox.Show(dcNodos[clave].Particion.ToString());
                                    if (tNd4.ImageKey != "nada" && tNd4.Level == 4)
                                    {
                                        ...
                                        //MessageBox.Show("DESDE: " + Vble.desde.ToString() + " - HASTA:" + Vble.hasta.ToString());
                                    }
                                    else
                                    {
        ...
        {
            int codconex;
            LabelCarga.Text = "Cargando Registros de Medidores";
            foreach (DataGridViewRow Fila in dataGridView1.Rows)//recorre cada registro de los nodos seleccionados
            {
                ...
                {                                              //va cargando el progress bar a medida que carga los registros
        ...
        /// <summary>
        /// Elimina los registros de las tablas SQLite
        /// </summary>
        public void ElimRegTabConexSQLite()
        {
            string txSQL;
        ...
        /// Elimina los registros de las tablas SQLite
        /// </summary>
        public void ElimRegTabMedSQLite()
        {
            string txSQL;
        ...
        private void button3_Click(object sender, EventArgs x)
        {
            ElimRegTabConexSQLite();
            ElimRegTabMedSQLite();
