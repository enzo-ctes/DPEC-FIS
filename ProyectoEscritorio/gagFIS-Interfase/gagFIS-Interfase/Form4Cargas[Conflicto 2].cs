...
namespace gagFIS_Interfase
{
    ...
    private void splitContainer1_Panel2_Paint(object sender, PaintEventArgs e)
        ...
        {                   
            ...
            try
            {
                txSQL = "select C.Secuencia as Nº_Secuencia, C.conexionID as Nº_Conexion, L.CodigoInt as Cod_Localidad, C.Ruta, C.usuarioID , P.Apellido, C.titularID, P.Apellido, C.propietarioID, P.apellido " +
                        "From conexiones C " +
        ...
        private DataTable CargarRegistrosSecuencia3()
        {
            ...
            try
            {
                txSQL = "select C.Secuencia, C.conexionID, L.CodigoInt, C.Ruta, C.usuarioID, P.Apellido as Apellido_Usuario, C.titularID, P.Apellido as Apellido_Titular, C.propietarioID, P.apellido as Apellido_Propietario " +
                        ...
                        "INNER JOIN localidades L ON C.CodPostalSumin = L.CodigoPostal " +
                        //"INNER JOIN conceptosdatos D on C.conexionID = D.conexionID " +
                        "Where (C.Secuencia >= " + ArrayDesde[0] + " and C.Secuencia <= " + ArrayHasta[0] + " and C.Ruta = " + ArrayRuta[0] + " and L.CodigoInt = " + Loc.Text + " and C.ImpresionOBS = " + 0 + 
                        " or C.Secuencia >= " + ArrayDesde[1] + " and C.Secuencia <= " + ArrayHasta[1] + " and C.Ruta = " + ArrayRuta[1] + " and C.ImpresionOBS = " + 0 + 
                        " or C.Secuencia >= " + ArrayDesde[2] + " and C.Secuencia <= " + ArrayHasta[2] + " and C.Ruta = " + ArrayRuta[2] + " and C.ImpresionOBS = " + 0 + ")";               
       ...
       private SQLiteConnection con;
                    ...
                    foreach (TreeNode tNd2 in tNd1.Nodes)
                    {
                        if (tNd2.ImageKey != "nada")
                        {
                            ...
                            foreach (TreeNode tNd3 in tNd2.Nodes)
                            {
                                ...
                                {
                                    if (tNd4.ImageKey == "todo" && tNd4.Level == 4)
                                    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show(ArrayDesde[0]+ " " + ArrayHasta[0]);
            //int i = 0;    
            //MessageBox.Show(ArrayDesde.Count.ToString());
            //if (ArrayDesde.Count > 0)
            //{                    
            //for (i = 0; i <= ArrayDesde.Count - 1; i++)
            //{
            //    MessageBox.Show("DESDE: " + ArrayDesde[i].ToString() + " - HASTA:" + ArrayHasta[i].ToString());
            //}
            //}
        }
