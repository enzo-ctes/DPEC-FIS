...
{
    public partial class FormResumenSup : Form
    {
        ...
        public FormResumenSup()
        {
            InitializeComponent();
        }

        private async void FormResumenSup_Load(object sender, EventArgs e)
        {
            ...
            if (DB.sDbUsu.ToUpper() == "SUPERVISOR")
            {
                ...
                RBRemesasIndiv.Enabled = true;
                CBRemesaIndiv.Enabled = true;
                RBRemesasIndiv.Checked = true;
                RBRemesasTodas.Enabled = true;
        ...
        private bool AgregarNodoDistritoExportadas(string sEmpresa, int Distrito)
        {
            ...
            tvExportadas.Nodes[Vble.Empresa.ToLower()].Expand();
        ...
        /// <param name="ConexionID"></param>
        /// <returns></returns>
        private void CargarNovedadesConex(Int32 ConexionID, int periodo, int j, string Operario, string ActualEstado)
        {
            DataTable Tabla2 = new DataTable();
            string obsernov = "";
            Vble.LineaHNC = "";
            ...
            try
            {
                ...
                txSQL = "select C.Lote, C.ConexionID, N.Codigo, N.Observ, N.Periodo " +
                           "From Conexiones C " +
                        ...
                        if (fi.Field<int>("ImpresionOBS") >= 500 & fi.Field<int>("ImpresionOBS") < 600 || fi.Field<int>("ImpresionOBS") < 800)
                        {
                            ...
                            foreach (DataColumn col in TablaUpload2.Columns)
                            {
                                ...
                                if (col.ColumnName == "ConexionID")
                                {
                                    ...
                                    else if (fi.Field<Int32>("ActualEstado") == -1)
                                    {
                                        ActualEstado = "";
                                    }
                                    else
                                    ...
                                    if (NroLinea == 0)
                                    {
                                        ...
                                        else if (EstadoImpresion > 1)
                                        {
                                            ...
                                            if (ExisteNovedades(fi.Field<int>("ConexionID"), fi.Field<int>("Periodo")))
                                            {
                                                CargarNovedadesConex(fi.Field<int>("ConexionID"), fi.Field<int>("Periodo"), j, Operario, ActualEstado);
                                            }
                                            else
                                            {
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            if (TextBoxRuta.Text != "")
            {
                ...
                if (DB.sDbUsu.ToUpper() == "SUPERVISOR")
                {
                    ...
                    CONSULTA = "SELECT DISTINCT C.Periodo, C.Remesa, C.Ruta, C.ConexionID AS NInstalacion, C.Contrato, " +
                                   ...
                                   "IF(C.ImpresionOBS MOD 100 = 0, '-', IF((date_format((str_to_date(I.Fec_Actual, '%d/%m/%Y')), '%Y-%m-%d') < date_format(M.ActualFecha, '%Y-%m-%d')), Concat('T: ', I.Lec_Actual), 'F')) as Tipo_Lectura, " +
                                   ...
                                   "WHERE ((C.ImpresionOBS >= 0 " + ") " +
                                   // "AND M.ActualFecha BETWEEN '" + FiltroDESDE + "' AND '" + FiltroHASTA + "') " +
                                   //" OR (C.ImpresionOBS MOD 100 >= 0 and M.ActualFecha BETWEEN '2000-01-01' and '2000-01-01') " +
                                   " AND C.Periodo = " + Vble.Periodo +
                                   " AND C.Remesa = " + CBRemesaRuta.Text + " " +
                                   ruta + ") " +
                                   //"OR (C.ImpresionOBS = 800 OR C.ImpresionOBS = 500 or C.ImpresionOBS = 400 or C.ImpresionOBS = 0) " +
                ...
                else
                {
                    CONSULTA = "SELECT DISTINCT C.Periodo, C.Remesa, C.Ruta, C.ConexionID AS NInstalacion, C.Contrato, " +
                              ...
                              "IF(N6.Observ = N62.Observ, N6.Observ, CONCAT(N6.Observ, ' ', N62.Observ))  as Observaciones  " +
                              "FROM Conexiones C " +
                              "INNER JOIN Personas P ON C.TitularID = P.PersonaID AND C.Periodo = P.Periodo " +
                              "INNER JOIN Errores E ON C.ImpresionOBS MOD 100 = E.Codigo " +
                              "INNER JOIN Medidores M ON C.ConexionID = M.ConexionID AND C.Periodo = M.Periodo  " +
            ...
            else
            {
                ...
                MessageBox.Show("Debe ingresar una ruta para poder realizar la busqueda individual.", "Datos Incompletos", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            ...
            if (seguir == true)
            {
                ...
                Funciones.VerDetallePreDescarga(LabelTodos.Text+"R", Vble.Periodo, CONSULTA, true, "999", REMESA,
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            string CONSULTA = "SELECT C.Periodo, C.Remesa, C.Ruta, C.ConexionID AS NInstalacion, " +
            ...
            Vble.TextHastaInformes = FiltroHASTA;
            //Funciones.VerDetallePreDescarga(LabLeidNoImprePre.Text, "602", Vble.Periodo, CONSULTA);
            Funciones.VerDetallePreDescarga(LabLeidNoImprePre.Text, Vble.Periodo, CONSULTA, true, "602", CBRemesaRuta.Text, FiltroDESDE, FiltroHASTA, TextBoxRuta.Text, "Resumen");
            this.Cursor = Cursors.Default;
        }


        //RESUMEN FUERA DE RANGO
        ...
        private void button8_Click_1(object sender, EventArgs e)
        {
            ...
            string CONSULTA = "SELECT DISTINCT C.Periodo, C.Ruta, C.ConexionID AS NInstalacion, C.Contrato, " +
                                     ...
                                     FiltroHASTA + "' AND C.Remesa = " + CBRemesaRuta.Text + ruta;
        ...
        private void button10_Click_1(object sender, EventArgs e)
        {
            ...
            CONSULTA = "SELECT L.Fecha, L.Hora, L.Periodo, L.Equipo, L.Ruta, L.Lecturista, C.titularID AS NUsuario, L.CodigoError, L.TextoError " +
        ...
        private void TextBoxDesde_Leave_1(object sender, EventArgs e)
        {
            if (TextBoxDesde.Text == "" || TextBoxDesde.TextLength != 10)
            {
                TextBoxDesde.Text = "dd/MM/yyyy";
        ...
        private void TextBoxDesde_Enter(object sender, EventArgs e)
        {
            if (TextBoxDesde.Text == "dd/MM/yyyy")
            {
                TextBoxDesde.Text = "";
        ...
        private void TextBoxHasta_Leave_1(object sender, EventArgs e)
        {
            if (TextBoxHasta.Text == "")
            {
                TextBoxHasta.Text = "dd/MM/yyyy";
        ...
        private void TextBoxHasta_KeyPress_1(object sender, KeyPressEventArgs e)
        {
            if (Char.IsDigit(e.KeyChar))
            {
                if (TextBoxHasta.TextLength == 2)
        ...
        private void button18_Click(object sender, EventArgs e)
        {
            ...
            //Verifico el tipo de filtro de remesa.
            //Si esta seleccionado el RBTodos, traera el informe de todas las remesas de las localidades asociadas en el archivo
            //ZonaFIS.txt
            //Si esta seleccionado el RBTodos, traera solo el informe de la remesa seleccionada en el CBRemesasIndividual.
            if (RBRemesasIndiv.Checked == true)
            {
                CONSULTA = "SELECT CAST(C.Remesa AS CHAR) AS Rem, " +
                        "CAST(C.Zona AS CHAR) AS Interfaz, " +
                        "CONCAT(CAST(C.Zona AS CHAR), '-', L.Localidad) AS Localidad, " +
                        "COUNT(*) AS 'TOTAL', " +
                        "SUM(IF(C.ImpresionCOD = 1, 1, 0)) AS InfoNoImprimir, " +
                        "SUM(IF(C.ImpresionOBS MOD 100 > 0, 1, 0)) AS Leidos, " +
                        "TRUNCATE(((SUM(IF(ImpresionOBS  >= 601 and ImpresionOBS < 800, 1, 0)) * 100) / COUNT(*)),2)  AS '% Tomados', " +
                        "SUM(IF(C.ImpresionOBS MOD 100 = 0, 1, 0)) AS Saldo, " +
                        "SUM(IF(SUBSTRING(C.ImpresionOBS,1,1) = 0, 1, 0)) AS 'Importados SIN procesar', " +
                        "SUM(IF(SUBSTRING(C.ImpresionOBS,1,1) = 3, 1, 0)) AS 'Procesados SIN salir', " +
                        "SUM(IF(SUBSTRING(C.ImpresionOBS,1,1) = 4, 1, 0)) AS 'EN CALLE', " +
                        "SUM(IF(SUBSTRING(C.ImpresionOBS,1,1) = 5, 1, 0)) AS 'Descargado NO exportado', " +
                        "SUM(IF(C.ImpresionOBS MOD 100 = 1, 1, 0)) AS Impresos, " +
                        "TRUNCATE(((SUM(IF(ImpresionOBS  = 601, 1, 0)) * 100) / COUNT(*)),2)  AS '% Impresos', " +
                        "SUM(IF(C.ImpresionOBS MOD 100 > 1, 1, 0)) AS LeidoNoImpresos, " +
                        "TRUNCATE(((SUM(IF(ImpresionOBS  > 601, 1, 0)) * 100) / COUNT(*)),2)  AS '% Leidos sin Imprimir'," +
                        "/*SUM(IF(C.ImpresionOBS MOD 100 = 0, '-', IF((date_format((str_to_date(I.Fec_Actual, '%d/%m/%Y')), '%Y-%m-%d') < date_format(M.ActualFecha, '%Y-%m-%d')), 1, 0))) as TeleLectura, */" +
                        "SUM(IF(C.ImpresionOBS MOD 100 = 4, 1, 0)) AS FueraRango, " +
                        "SUM(IF(ImpresionOBS MOD 100 = 15, 1, 0)) AS CorreccionEstado, " +
                        "SUM(IF(C.ImpresionOBS MOD 100 = 16, 1, 0)) AS Ordenativo, " +
                        "SUM(IF(ImpresionOBS MOD 100 = 2, 1, 0)) AS LeidoEsperandoRespuesta, " +
                        "SUM(IF(ImpresionOBS MOD 100 = 20, 1, 0)) AS ErrorFacturando, " +
                        "SUM(IF(ImpresionOBS MOD 100 = 23, 1, 0)) AS PeriodoExcedido, " +
                        "SUM(IF((C.ImpresionOBS MOD 100 > 39 AND ImpresionOBS MOD 100 < 50), 1, 0)) AS Impresora, " +
                        "SUM(IF(((C.ImpresionOBS MOD 100 > 49 AND ImpresionOBS MOD 100 < 60) OR ImpresionOBS MOD 100 = 8), 1, 0)) AS Indicado, " +                        
                        "SUM(IF(C.ImpresionOBS MOD 100 = 9, 1, 0)) AS Imposible, " +
                        //"SUM(IF((C.ImpresionOBS MOD 100 = 9 AND ImpresionCOD = 1), 1, 0)) AS 'Indicado E Imposible', " +
                        "SUM(IF((C.ImpresionOBS MOD 100 > 69 AND C.ImpresionOBS MOD 100 < 78), 1, 0)) AS SAP, " +
                        "SUM(IF(((C.ImpresionOBS MOD 100 > 77 AND C.ImpresionOBS MOD 100 < 80) OR C.ImpresionOBS MOD 100 = 26), 1, 0)) AS WS " +
                        "FROM  Conexiones C " +
                        //"JOIN Medidores M ON C.ConexionID = M.ConexionID AND C.Periodo = M.Periodo "+
                        "JOIN Localidades L ON C.Zona = L.CodigoInt " +
                        "/*INNER JOIN Impresor I ON C.ConexionID = I.ConexionID AND C.Periodo = I.Periodo */"+
                        "WHERE ((C.Periodo = " + Vble.Periodo + " AND Remesa = " + CBRemesaIndiv.Text + ")) AND C.ImpresionOBS < 898 " +
                        "GROUP BY C.Zona , Remesa ORDER BY Interfaz, Zona, Remesa, Localidad;";

                Vble.TextDesdeInformes = FiltroDESDE;
                Vble.TextHastaInformes = FiltroHASTA;

                REMESA = CBRemesaIndiv.Text;

                Funciones.VerDetallePreDescargaR(LabelTodosRem.Name, Vble.Periodo, CONSULTA, true, "999", REMESA,
                                                TextBoxRuta.Text, "ResumenRemesa", LabelTodosRem.Text, "R");
              
            }
            else if(RBRemesasTodas.Checked == true)
            {
                CONSULTA = "SELECT CAST(C.Remesa AS CHAR) AS Rem, " +
                            "CAST(C.Zona AS CHAR) AS Interfaz, " +
                            "CONCAT(CAST(C.Zona AS CHAR), '-', L.Localidad) AS Localidad, " +
                            "COUNT(*) AS 'Total', " +
                            "SUM(IF(C.ImpresionCOD = 1, 1, 0)) AS InfoNoImprimir, " +
                            "SUM(IF(C.ImpresionOBS MOD 100 > 0 and C.ImpresionOBS MOD 100 < 80, 1, 0)) AS Leidos, " +
                            "TRUNCATE(((SUM(IF(ImpresionOBS  >= 601 and ImpresionOBS < 800, 1, 0)) * 100) / COUNT(*)),2)  AS '% Tomados', " +
                            "SUM(IF(C.ImpresionOBS MOD 100 = 0, 1, 0)) AS Saldo, " +
                            "SUM(IF(SUBSTRING(C.ImpresionOBS,1,1) = 0, 1, 0)) AS 'Importados SIN procesar', " +
                            "SUM(IF(SUBSTRING(C.ImpresionOBS,1,1) = 3, 1, 0)) AS 'Procesados SIN salir', " +
                            ...
                            "JOIN Localidades L ON C.Zona = L.CodigoInt " +
                            "/*INNER JOIN Impresor I ON C.ConexionID = I.ConexionID AND C.Periodo = I.Periodo */"+
                            "WHERE ((Periodo = " + Vble.Periodo + " AND (Remesa = 1 OR Remesa = 2 OR Remesa = 3 OR Remesa = 4 OR Remesa = 5 OR Remesa = 6 OR Remesa = 7 OR Remesa = 8))) AND ImpresionOBS < 898 " +
                            "GROUP BY C.Zona , Remesa ORDER BY Interfaz, Zona, Remesa, Localidad;";
                Vble.TextDesdeInformes = FiltroDESDE;
                Vble.TextHastaInformes = FiltroHASTA;
                ...
                Funciones.VerDetallePreDescargaR(LabelTodosRem.Name, Vble.Periodo, CONSULTA, true, "9999", REMESA,
                                                TextBoxRuta.Text, "ResumenRemesa", LabelTodosRem.Text, "T");
            }
        ...
        {
            if (RBRemesasIndiv.Checked == false)
            {
                RBRemesasIndiv.Checked = true;
                CBRemesaIndiv.Enabled = true;
        ...
        private void button21_Click(object sender, EventArgs e)
        {
            ...
            if (RBRemesasIndiv.Checked == true)//Si el check de Todos en el filtro por remesas esta sin seleccionar ingresa por aca, para filtrar solamente la remesa seleccionada en el combo individual.
            {
                CONSULTA = "SELECT DISTINCT C.Periodo, C.Zona, C.Remesa, C.Ruta, C.ConexionID AS NInstalacion, C.Contrato, " +
                               "C.titularID AS IC, P.Apellido, M.Numero as Medidor, C.DomicSumin as Domicilio, M.AnteriorEstado, M.ActualEstado, C.ConsumoFacturado, M.ActualFecha AS Fecha, M.ActualHora AS Hora, " +
                               "C.Operario " +                               
                               "FROM Conexiones C INNER JOIN Personas P ON C.TitularID = P.PersonaID AND C.Periodo = P.Periodo " +
                               "INNER JOIN Medidores M ON C.ConexionID = M.ConexionID AND C.Periodo = M.Periodo " +                               
                               "WHERE (C.ImpresionOBS MOD 100 = 9)" +
                               "AND C.Remesa = " + CBRemesaIndiv.Text + " and C.Periodo = " + Vble.Periodo;
                Vble.TextDesdeInformes = FiltroDESDE;
                Vble.TextHastaInformes = FiltroHASTA;
             
            }
            else if (RBRemesasTodas.Checked == true)//Si el check de Todos en el filtro por remesas esta seleccionado ingresa por aca, para filtrar por todas las remesas.
            {
                CONSULTA = "SELECT DISTINCT C.Periodo, C.Remesa, C.Ruta, C.ConexionID AS NInstalacion, C.Contrato, " +
                               "C.titularID AS IC, P.Apellido, M.Numero as Medidor, C.DomicSumin as Domicilio, M.AnteriorEstado, M.ActualEstado, C.ConsumoFacturado, M.ActualFecha AS Fecha, M.ActualHora AS Hora, " +
                               "C.Operario " +
                               ...
                               "INNER JOIN Medidores M ON C.ConexionID = M.ConexionID AND C.Periodo = M.Periodo " +
                               "WHERE (C.ImpresionOBS MOD 100 = 9)" +
                               "AND (C.Remesa = 1 OR C.Remesa = 2 OR C.Remesa = 3 OR C.Remesa = 4 OR C.Remesa = 5 OR C.Remesa = 6 OR C.Remesa = 7 OR C.Remesa = 8) and C.Periodo = " + Vble.Periodo;
                Vble.TextDesdeInformes = FiltroDESDE;
                Vble.TextHastaInformes = FiltroHASTA;
            ...
            Funciones.VerDetallePreDescargaR(labelLectImpR.Name, Vble.Periodo, CONSULTA, true, "609", REMESA,
                                            TextBoxRuta.Text, "ResumenRemesa", labelLectImpR.Text);
        ...
        private void button19_Click(object sender, EventArgs e)
        {
            ...
            if (RBRemesasIndiv.Checked == true)//Si el check de Todos en el filtro por remesas esta sin seleccionar ingresa por aca, para filtrar solamente la remesa seleccionada en el combo individual.
            {
                //Periodo|Ruta|NInstalacion|Contrato|IC|Apellido|Domicilio|Medidor|AnteriorEstado|ActualEstado|Fecha|Hora|ConsumoFacturado|Operario|Ord1|Ord2|Ord3|Ord4|Ord5|Observaciones
                CONSULTA = "SELECT DISTINCT C.Periodo, C.Zona, C.Ruta, C.Remesa as Rem, C.ConexionID AS NInstalacion, C.Contrato, " +
                                     "C.titularID AS IC, C.DomicSumin as Domicilio, M.Numero AS Medidor, M.AnteriorEstado, " +
                                     "M.ActualEstado, M.ActualFecha as Fecha, M.ActualHora AS Hora, C.ConsumoFacturado, " +
                                     "C.Operario " +
                                     //"N1.Codigo as Ord1, N2.Codigo as Ord2, N3.Codigo as Ord3, N4.Codigo as Ord4, N5.Codigo as Ord5, N6.Observ as Observaciones " +
                                     "FROM Conexiones C " +
                                     ...
                                     //"ON N6.ConexionID = C.ConexionID AND N6.Periodo = C.Periodo " +
                                     "WHERE " +
                                     "C.ImpresionCOD = 1 AND C.Remesa = " + CBRemesaIndiv.Text + " AND C.Periodo = " + Vble.Periodo + " ORDER BY C.Zona ASC";
            }
            else if (RBRemesasTodas.Checked == true)//Si el check de Todos en el filtro por remesas esta seleccionado ingresa por aca, para filtrar por todas las remesas.
            {
                //CONSULTA = "SELECT DISTINCT C.Periodo, C.Zona, C.Ruta, C.Remesa as Rem, C.ConexionID AS NInstalacion, C.Contrato, " +
                ...
                //                     "ON N6.ConexionID = C.ConexionID AND N6.Periodo = C.Periodo " +
                //                     "WHERE " +
                //                     "C.ImpresionCOD = 1 AND (C.Remesa = 1 OR C.Remesa = 2 OR C.Remesa = 3 OR C.Remesa = 4 OR C.Remesa = 5 OR C.Remesa = 6 OR C.Remesa = 7 OR C.Remesa = 8)  AND C.Periodo = " + Vble.Periodo + " ORDER BY C.Zona ASC";
            ...
            Funciones.VerDetallePreDescargaR(LabIndNoPrintR.Name, Vble.Periodo, CONSULTA, true, "1", REMESA,
                                            TextBoxRuta.Text, "ResumenRemesa", LabIndNoPrintR.Text);
            ...
            //Funciones.VerDetallePreDescarga(LabIndNoPrint.Text, Vble.Periodo, CONSULTA, true, "1", CBRemesaIndiv.Text, FiltroDESDE, FiltroHASTA, TextBoxRuta.Text, "Resumen");
            this.Cursor = Cursors.Default;
        }

        //Resumen Imprsas POR REMESA
        private void button14_Click(object sender, EventArgs e)
        {
            ...
            if (RBRemesasIndiv.Checked == true)//Si el check de Todos en el filtro por remesas esta sin seleccionar ingresa por aca, para filtrar solamente la remesa seleccionada en el combo individual.
            {
                //Periodo|Remesa|Ruta|NInstalacion|Contrato|IC|Apellido|Medidor|Domicilio|AnteriorEstado|ActualEstado|Fecha|Hora|ConsumoFacturado|Operario|Ord1|Ord2|Ord3|Ord4|Ord5|Observaciones
                 CONSULTA = "SELECT DISTINCT C.Periodo, C.Zona, C.Remesa, C.Ruta, C.ConexionID AS NInstalacion, C.Contrato, " +
                              "C.titularID AS IC, CONCAT(P.Apellido, ' ', P.Nombre) as Apellido, M.Numero AS Medidor, C.DomicSumin as Domicilio, " +
                              "M.AnteriorEstado, M.ActualEstado, M.ActualFecha AS Fecha, M.ActualHora AS Hora, C.ConsumoFacturado, C.Operario, " +
                              "N1.Codigo as Ord1, N2.Codigo as Ord2, N3.Codigo as Ord3, N4.Codigo as Ord4, N5.Codigo as Ord5, N6.Observ as Observaciones " +
            ...
            else if (RBRemesasTodas.Checked == true)//Si el check de Todos en el filtro por remesas esta seleccionado ingresa por aca, para filtrar por todas las remesas.
            {
                ...
                CONSULTA = "SELECT DISTINCT C.Periodo, C.Zona, C.Remesa, C.Ruta, C.ConexionID AS NInstalacion, C.Contrato, " +
                              ...
                              "INNER JOIN Medidores M ON C.ConexionID = M.ConexionID AND C.Periodo = M.Periodo " +
                              "WHERE (ImpresionOBS MOD 100 = 1) " +
                              " AND(C.Remesa = 1 OR C.Remesa = 2 OR C.Remesa = 3 OR C.Remesa = 4 OR C.Remesa = 5 OR C.Remesa = 6 OR C.Remesa = 7 OR C.Remesa = 8)  AND C.Periodo = " + Vble.Periodo + " ORDER BY C.Zona ASC";
            }
            ...
            //Funciones.VerDetallePreDescarga(LabImprPre.Text, "601", Vble.Periodo, CONSULTA);
            Funciones.VerDetallePreDescargaR(LabImprPreR.Name, Vble.Periodo, CONSULTA, true, "601", REMESA,
                                            TextBoxRuta.Text, "ResumenRemesa", LabImprPreR.Text);
            this.Cursor = Cursors.Default;
        }
        ...
        private void button16_Click(object sender, EventArgs e)
        {
            ...
            if (RBRemesasIndiv.Checked == true)//Si el check de Todos en el filtro por remesas esta sin seleccionar ingresa por aca, para filtrar solamente la remesa seleccionada en el combo individual.
            {
                ...
                CONSULTA = "SELECT C.Periodo, C.Zona, C.Remesa, C.Ruta, C.ConexionID AS NInstalacion, " +
                              "C.Contrato, C.titularID AS IC, P.Apellido, M.Numero as Medidor,  C.DomicSumin as Domicilio, " +
                              ...
                              "WHERE ((ImpresionOBS MOD 100 > 1) AND (ImpresionOBS MOD 100 < 99)) " +
                              "AND C.Remesa = " + CBRemesaIndiv.Text + " AND C.Periodo = " + Vble.Periodo;
            }
            else if (RBRemesasTodas.Checked == true)//Si el check de Todos en el filtro por remesas esta seleccionado ingresa por aca, para filtrar por todas las remesas.
            {
            ...
            Vble.TextHastaInformes = FiltroHASTA;
            //Funciones.VerDetallePreDescarga(LabLeidNoImprePre.Text, "602", Vble.Periodo, CONSULTA);
            Funciones.VerDetallePreDescargaR(LabLeidNoImprePreR.Name, Vble.Periodo, CONSULTA, true, "602", REMESA,
                                            TextBoxRuta.Text, "ResumenRemesa", LabLeidNoImprePreR.Text);
            this.Cursor = Cursors.Default;
        }
        ...
        private void button17_Click(object sender, EventArgs e)
        {
            ...
            if (RBRemesasIndiv.Checked == true)//Si el check de Todos en el filtro por remesas esta sin seleccionar ingresa por aca, para filtrar solamente la remesa seleccionada en el combo individual.
            {
                CONSULTA = "SELECT DISTINCT C.Periodo, C.Zona, C.Remesa, C.Ruta, C.ConexionID AS NInstalacion, C.Contrato, " +
                              ...
                              "FROM Conexiones C INNER JOIN Personas P ON C.TitularID = P.PersonaID AND C.Periodo = P.Periodo " +
                              "INNER JOIN Medidores M ON C.ConexionID = M.ConexionID AND C.Periodo = M.Periodo " +                             
                              " WHERE (C.ImpresionOBS MOD 100 = 4) " +
                              "AND C.Remesa = " + CBRemesaIndiv.Text + " AND C.Periodo = " + Vble.Periodo;
            }
            else
            {
            ...
            //Funciones.VerDetallePreDescarga(LabNoImprFueraRango.Text, "604", Vble.Periodo, CONSULTA);
            Funciones.VerDetallePreDescargaR(labelFuerRangR.Name, Vble.Periodo, CONSULTA, true, "604", REMESA,
                                            TextBoxRuta.Text, "ResumenRemesa", labelFuerRangR.Text);
            this.Cursor = Cursors.Default;
        }
        ...
        private void button20_Click(object sender, EventArgs e)
        {
            ...
            if (RBRemesasIndiv.Checked == true)//Si el check de Todos en el filtro por remesas esta sin seleccionar ingresa por aca, para filtrar solamente la remesa seleccionada en el combo individual.
            {
                CONSULTA = "SELECT DISTINCT C.Periodo, C.Zona, C.Remesa, C.Ruta, C.ConexionID AS NInstalacion, C.Contrato, C.titularID AS IC, " +
                              "C.DomicSumin, P.Apellido, M.Numero AS Medidor, C.DomicSumin as Domicilio, M.AnteriorEstado, M.ActualEstado, C.ConsumoFacturado, " +
                              "M.ActualFecha AS Fecha, M.ActualHora AS Hora, C.Operario " +
                              "FROM Conexiones C INNER JOIN Personas P ON C.TitularID = P.PersonaID AND C.Periodo = P.Periodo " +
                              "INNER JOIN Medidores M ON C.ConexionID = M.ConexionID AND C.Periodo = M.Periodo " +
            ...
            //Funciones.VerDetallePreDescarga(LabSaldos.Text, "", Vble.Periodo, CONSULTA);
            Funciones.VerDetallePreDescargaR(LabLeidNoImprePreR.Name, Vble.Periodo, CONSULTA, true, "0", REMESA,
                                          TextBoxRuta.Text, "ResumenRemesa", LabLeidNoImprePreR.Text);
            this.Cursor = Cursors.Default;
        }
        ...
        private void button22_Click(object sender, EventArgs e)
        {
            ...
            if (RBRemesasIndiv.Checked == true)//Si el check de Todos en el filtro por remesas esta sin seleccionar ingresa por aca, para filtrar solamente la remesa seleccionada en el combo individual.
            {
                CONSULTA = "SELECT DISTINCT C.ConexionID, C.Zona, IF(ImpresionOBS >= 400 and ImpresionOBS < 500, 'EN CALLE', " +
                    "IF(ImpresionOBS >= 500 and ImpresionOBS < 600, 'DESCARGADO NO EXPORTADO', IF(ImpresionOBS >= 600 and ImpresionOBS < 700, " +
                    "'EXPORTADO', IF(ImpresionOBS = 0, 'NO LEIDO', 'OTROS')))) AS Situacion, E.Titulo as Estado, " +
                           " N1.Codigo as 'ORD1', N2.Codigo as 'ORD2', N3.Codigo as 'ORD3', N4.Codigo as 'ORD4', N5.Codigo as 'ORD5', N6.Observ as 'Observacion' FROM Conexiones C " +
                           "left JOIN(SELECT ConexionID, Periodo, Codigo FROM NovedadesConex WHERE Orden = 1 and Periodo = " + Vble.Periodo + ") N1 " +
                           "ON N1.ConexionID = C.ConexionID AND N1.Periodo = C.Periodo " +
                              "left JOIN(SELECT ConexionID, Periodo, Codigo FROM NovedadesConex WHERE Orden = 2 and Periodo = " + Vble.Periodo + ") N2 " +
                              "ON N2.ConexionID = C.ConexionID AND N2.Periodo = C.Periodo "+
                             ...
                             "ON N6.ConexionID = C.ConexionID AND N6.Periodo = C.Periodo " +
                              "INNER JOIN Errores E ON C.ImpresionOBS MOD 100 = E.Codigo " +
                             "WHERE C.Remesa = " + CBRemesaIndiv.Text + " AND C.Periodo = " + Vble.Periodo + " AND (N1.Codigo = 1 OR N1.Codigo = 26 " +
                                    "OR N1.Codigo = 66 OR N1.Codigo = 67 OR N1.Codigo = 82 " +
                                    "OR N1.Codigo = 84 " +
                                    "OR N1.Codigo = 5 OR N1.Codigo = 45) ORDER BY C.Zona"; 
            }
            else if (RBRemesasTodas.Checked == true)//Si el check de Todos en el filtro por remesas esta seleccionado ingresa por aca, para filtrar por todas las remesas.
            {
                CONSULTA = "SELECT DISTINCT C.ConexionID, C.Zona, IF(ImpresionOBS >= 400 and ImpresionOBS < 500, 'EN CALLE', " +
                    ...
                    "'EXPORTADO', IF(ImpresionOBS = 0, 'NO LEIDO', 'OTROS')))) AS Situacion, E.Titulo as Estado, " +
                           ...
                           "ON N1.ConexionID = C.ConexionID AND N1.Periodo = C.Periodo " +
                             ...
                             "ON N6.ConexionID = C.ConexionID AND N6.Periodo = C.Periodo " +
                              "INNER JOIN Errores E ON C.ImpresionOBS MOD 100 = E.Codigo " +
                             "WHERE (C.Remesa = 1 OR C.Remesa = 2 OR C.Remesa = 3 OR C.Remesa = 4 OR C.Remesa = 5 OR C.Remesa = 6 OR C.Remesa = 7 OR C.Remesa = 8) AND C.Periodo = " + Vble.Periodo + 
                                    " AND (N1.Codigo = 1 OR N1.Codigo = 26 " +
                                    "OR N1.Codigo = 66 OR N1.Codigo = 67 OR N1.Codigo = 82 " +
                                    "OR N1.Codigo = 84 " +
                                    "OR N1.Codigo = 5 OR N1.Codigo = 45) ORDER BY C.Zona";
            ...
            Funciones.VerDetallePreDescargaR(lblOrdenativosR.Name, Vble.Periodo, CONSULTA, true, "616", REMESA,
                                            TextBoxRuta.Text, "ResumenRemesa", lblOrdenativosR.Text);
            this.Cursor = Cursors.Default;
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public void RecorredNodos(object sender, EventArgs e)
        {
            try
            {
                int Lote = 0, Ruta = 0, Distr = 0, Seleccionados = 0, Remesa = 0, Disponibles = 0;
        ...
        private void LabImprPreR_Click(object sender, EventArgs e)
        {
        ...
        private void labelLectImpR_Click(object sender, EventArgs e)
        {
        ...
        private void button23_Click(object sender, EventArgs e)
        {
            ...
            if (RBRemesasIndiv.Checked == true)
            {
                CONSULTA = "SELECT  CAST(C.Zona AS CHAR) AS Interfaz, "+
                            "CAST(C.Remesa AS CHAR) AS Rem, "+
                            "CONCAT(CAST(C.Zona AS CHAR), '-', L.Localidad) AS Localidad, "+
                            "COUNT(*) AS 'TOTAL', "+
                            "SUM(IF(C.ImpresionOBS MOD 100 > 0, 1, 0)) AS Leidos, "+
                            "SUM(IF(C.ImpresionOBS MOD 100 = 1, 1, 0)) AS Impresos, "+
                            "SUM((IF((DATE_FORMAT((STR_TO_DATE(I.Fec_Actual, '%d/%m/%Y')), "+
                            "'%Y-%m-%d') < DATE_FORMAT(M.ActualFecha, '%Y-%m-%d')), "+
                            "1, 0))) AS TeleLectura, "+
                            "SUM(IF(C.ImpresionOBS MOD 100 = 0, 1, 0)) AS Saldo "+
                            "FROM impresor I "+
                            "left JOIN Medidores M "+
                            "USING(ConexionID, Periodo) "+
                            "RIGHT JOIN Conexiones C "+
                            "USING(ConexionID, Periodo) "+
                            "INNER JOIN " +
                            "Localidades L ON C.Zona = L.CodigoInt "+
                            "WHERE ((C.Periodo = " + Vble.Periodo + " AND Remesa = " + CBRemesaIndiv.Text + ")) AND C.ImpresionOBS < 898 " +
                            "GROUP BY C.Zona , Remesa ORDER BY Zona, Remesa, Localidad;";
                ...
                Funciones.VerDetallePreDescargaR(LabelTeleLect.Name, Vble.Periodo, CONSULTA, true, "999", REMESA,
                                                TextBoxRuta.Text, "ResumenRemesa", LabelTeleLect.Text, "R");
            }
            else if (RBRemesasTodas.Checked == true)
            {
                CONSULTA = "SELECT  CAST(C.Zona AS CHAR) AS Interfaz, "+
                ...
                Funciones.VerDetallePreDescargaR(LabelTeleLect.Name, Vble.Periodo, CONSULTA, true, "9999", REMESA,
                                                TextBoxRuta.Text, "ResumenRemesa", LabelTeleLect.Text, "T");
            }

        }

        private void LabelTeleLect_Click(object sender, EventArgs e)
        {
