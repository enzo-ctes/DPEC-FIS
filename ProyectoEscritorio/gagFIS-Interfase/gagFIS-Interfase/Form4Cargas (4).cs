...
namespace gagFIS_Interfase
{
    ...
    public partial class Form4Cargas : Form {
        ...
        private void form4_Load(object sender, System.EventArgs e) {
            ...
            this.toolTip1.SetToolTip(this.BotEnviarCarga, "Envia los archivos seleccionados a la Colectora");
            this.toolTip2.SetToolTip(this.BotActPanPC, "Actualiza el Panel de Cargas Procesadas");
       ...
       //Boton que cierra el formulario actual
        ...
        private void GenerarCarpetaArchivo() {
            ...
            CrearDirectorioVacio(ArchivoTabla);
        ...
        private void CrearArchivoInfoCarga(string archivosecuencia, string filename)
        {
            try
            {
                ...
                {
                    Vble.lineas += ArrayLocalidad[i] + "-" + ArrayRuta[i] + " (" + ArrayDesde[i] + "-" + ArrayHasta[i] + ") " + ArrayCantConex[i] + "\n";
                }               
    ...
    private void splitContainer1_Panel2_Paint(object sender, PaintEventArgs e)
                ...
                for (int i = 0; i < ArrayDesde.Count; i++)
                    {
                        ...
                        " and L.CodigoInt = " + ArrayLocalidad[i] + " and C.ImpresionOBS = " + 0 + " OR  C.ImpresionOBS = " + 500 + ")";                
                    }
      ...
      txSQL = "INSERT INTO ConceptosDatos ([conexionID], [Periodo], [CodigoConcepto], [CodigoEscalon], [CodigoAux], [CodigoGrupo], [TextoEscalon]," +
            ...
            //if (this.dataGridView1.RowCount > 0)//verifica que en el datagridview1 existan registros para recorrerlos y generar la carga de las tablas
              if (Vble.TablaConexSelec.Rows.Count > 0)
                ...
                foreach (DataRow Fila in Vble.TablaConexSelec.Rows)
                    ...
                    Vble.CambiarEstadoConexion(codconex, Convert.ToInt32(cteCodEstado.ParaCargar));
        ...
        /// <summary>
        /// Metodo que Muestra en el ListView1 todas las cargas procesadas de las distintas localidades
        /// </summary>
        public void CargasProcesadas()
        {
            ...
            DirectoryInfo di = new DirectoryInfo(Vble.CarpetasCargasAcargar);
            if (Directory.Exists(di.ToString()))
            {
           
            foreach (var dis in di.GetDirectories())
            {
               //MessageBox.Show(dis.Name);
                            ...
                            ListViewItem Datos = new ListViewItem(fi.Name);
                                ...
                                foreach (var files in fi.GetFiles())
                                {
                                    if (files.Name.Contains("InfoCarga.txt"))
                                    {
                                        ...
                                        //                                //Agrega el tooltip de información de las cargas generadas.
                                        using (StreamReader sr = new StreamReader(files.DirectoryName + "\\InfoCarga.txt", false))
                                        //using (StreamWriter rs = new StreamWriter(files.DirectoryName + "\\InfoCarga.txt", false))
                                        {
                                            string line, linTool;
                                            linTool = "";
                                            while ((line = sr.ReadLine()) != null)
                                ...
                                //ArrayCarpetasCargas.Add(di.FullName);
                            }
        ...
        /// <summary>
        /// Metodo que obtendra la ruta Origen para la copia de archivos que se pasaran a la subcarpeta 
        ...
        /// Copiar archivo
        /// </summary>
        public void CopiaArchivos(string RutaOrigen, string RutaDestino)
        {
            try
            {
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    device.Connect();
                    if (BuscarColectora(device.ToString()))
                    {
        ...
        private void EnviarArchivos()
        {
            ...
            Dispositivo = BuscarNombreColectora(Vble.DirectorioColectoraenPC);
            ...
            //*string Origen1 = shellView1.SelectedItems.First().FileSystemPath;                                           
            string Origen1 = Vble.RutaCarpetaOrigen;

            //ruta del directorio donde se va a enviar el archivo seleccionado
            //*string path1 = Ruta + shellView1.SelectedItems.First().DisplayName;
            string Destino1 = Ruta + Vble.CarpetaSeleccionada;
            ...
            if (!Directory.Exists(RutaEnviadas()))
                CrearDirectorioVacio(RutaEnviadas());
            ...
            if (!Directory.Exists(Vble.CarpetaRespaldo))
                CrearDirectorioVacio(Vble.CarpetaRespaldo + Vble.RespaldoEnviadas);
                CopiarDirectorio(Origen1, Vble.CarpetaRespaldo + Vble.RespaldoEnviadas, true);

            CargasEnviadas();
            //ListViewItem Datos = new ListViewItem(Vble.CarpetaSeleccionada);
            //ListViewCargados.Items.Add(Datos);
            ...
            if (Directory.Exists(Destino1))
                Directory.Delete(Destino1);
                Directory.Move(Origen1, Destino1);
            //if (!Directory.Exists(Destino1))
            //    CrearDirectorioVacio(Destino1);
                      ...
                      if ((shellView2.Visible == true))
                        {
                          if (shellView2.Enabled == false)
                            ...
                            Vble.CambiarEstadoConexionSqlite(Convert.ToInt32(cteCodEstado.Cargado));
                            ...
                            foreach (var carpeta in raiz)
                            {
                                ...
                                if (carpeta.DisplayName == Vble.DestinoArchivosColectora)
                                {
                                    ...
                                    foreach (var destino in carpeta)
                                    {
                                        ...
                                        if (destino.DisplayName == "Datos DPEC")
